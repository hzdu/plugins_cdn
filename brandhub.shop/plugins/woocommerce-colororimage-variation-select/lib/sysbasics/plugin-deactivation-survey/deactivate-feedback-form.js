(function($) {
	...
	sysbasics.DeactivateFeedbackForm = function(plugin)
	{
			...
			<div class="sysbasics-deactivate-dialog" data-remodal-id="' + plugin.slug + '">\
				<form>\
					...
					<p class="sysbasics-deactivate-dialog-buttons">\
						<input type="submit" class="skipanddeactivate button confirm" value="' + strings.skip_and_deactivate + '"/>\
						<button data-remodal-action="cancel" class="button button-primary">' + strings.cancel + '</button>\
						<a href="https://wordpress.org/support/plugin/color-image-variation-swatches-for-woocommerce/#new-topic-0" class="button button-success"/>Contact Support</a>\
					</p>\
				</form>\
			</div>\
		')[0];
		...
		$(element).on("click", ".skipanddeactivate", function(event) {
