		...
		$(document).ready(function () {
			$(".urcr-conditional-logic-definitions").each(function () {
				...
				if ($("#" + Id).find(".ur-form-group").length <= 0) {
					...
					$("#" + Id).find('.ucr-logic-group-rule').children('span').removeAttr('Class');
					$("#" + Id).find('.ucr-logic-group-rule').removeAttr('Class');
				}
			});
		});
	...
	var urcr_utils = {
		...
		create_switch_tag: function (args) {
			...
			html +=
				...
				" </label>";
			html += "</div>";
			html +='<div class="urcr-notice urcr-notice-warning"><p><b>Warning:</b> It will restrict the whole site.Please add the login shortcode in the action tab.</p></div>';
				...
				function (e) {
					...
					if (!$(this).is(".nav-tab-active")) {
						...
						if (tab_content_selector) {
							...
							$(tab_content_selector).slideDown(300);
