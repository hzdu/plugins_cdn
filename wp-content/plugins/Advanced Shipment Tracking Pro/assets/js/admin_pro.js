...
/*jQuery(document).on("submit", "#tpi_settings_form", function( ){	
	...
	jQuery.ajax({
		...
		success: function(response) {
			...
			jQuery("#ast_settings_snackbar").text(ast_pro_ajax_object.i18n.data_saved);			
...
jQuery(document).on("click", "#wc_ast_status_new_shipped", function(){
	if ( jQuery(this).prop("checked") == true ) {
		...
		jQuery.ajax({
			...
			type: 'POST',
			success: function(response) {
				jQuery('.shipped_checkbox').show();	
				jQuery('.shipped_checkbox input[type="checkbox"]').prop('checked', true);	
			},
			error: function(response) {				
			}
		});			
