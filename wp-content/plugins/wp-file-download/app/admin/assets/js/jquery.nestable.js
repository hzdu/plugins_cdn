...
(function($, window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    dragStop: function(e) {
      ...
      this.dragRootEl.removeClass('dd-dragging');
      this.options.callback.call(this, this.dragRootEl, el, position);

      this.reset();

      this.options.afterDragStop.call(this, this.el, el, this.placeEl.parent())
    },
    ...
    dragMove: function(e) {
      ...
      // Force drop over the icon
      if (this.pointEl.hasClass('wpfd-folder')) {
        isOver = true;
      ...
      isOver = isOver || (e.pageY > (this.pointEl.offset().top + 3) && e.pageY < (this.pointEl.offset().top + this.pointEl.height() - 3));

      if (isOver) {
        this.dragRootEl.addClass('dd-dragging');
      }
      /**
       * move vertical
       */
      if (!mouse.dirAx || isNewRoot || isEmpty || isOver) {
        ...
        // Calculate the before, after and on drag position
        var before = e.pageY < (this.pointEl.offset().top + 3);
        ...
        if (isEmpty && !isOver) {
          ...
          if (!list.length) {
            this.createSubLevel(this.pointEl, this.placeEl);
          }
          else {
            // else append to next level up
