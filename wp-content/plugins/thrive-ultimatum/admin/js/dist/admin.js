...
( function ( $ ) {
	...
	ThriveUlt.util.leadtype = {
		...
		thriveBox: 'tve_lead_2s_lightbox'
	};
	...
	ThriveUlt.util.select2ImageItemRenderer = function ( option ) {
		if ( option.element && option.element.dataset.image ) {
			return $( '<span><span class="opt-image" style="background-image: url(' + option.element.dataset.image + ')"></span><span>' + option.text + '</span></span>' );
		}
		return option.text;
	};

	/**
	 *  Measure the string given the font parameters and trim it to match a given width
	 *
	 * @param {String} text
	 ...
	 * @returns {string}
	 */
	ThriveUlt.util.trimStringAfterWidth = function ( text, width, fontSize, fontFamily, fontWeight ) {
		//Create a temporary span to measure the text
		var span = document.createElement( 'span' );
		span.style.cssText = 'visibility: hidden; float: left; font-size: ' + fontSize + 'px; font-family: ' + fontFamily + '; font-weight: ' + fontWeight + '; ';
		document.body.appendChild( span );
		span.innerHTML = text;

		var ifTooLong = span.offsetWidth > width;
		//Reduce characters until the text is <= then the desired width
...
( function ( $ ) {
	...
	ThriveUlt.models.Campaign = ThriveUlt.models.Base.extend( {
		...
		validate_lockdown: function ( attrs, errors ) {
			...
			if ( ! s.promotion || s.promotion.length === 0 || ( s.promotion instanceof Object && ! s.promotion.at( 0 ).get( 'id' ) && ! s.promotion.at( 0 ).get( 'value' ) ) ) {
		...
		validate_evergreen: function ( attrs, errors ) {
			var s = attrs.settings,
				daysDuration = parseInt( s.days_duration ),
				hoursDuration = parseInt( s.hours_duration ),
				minutesDuration = parseInt( s.minutes_duration ),
				secondsDuration = parseInt( s.seconds_duration ),
				totalDuration = daysDuration + hoursDuration + minutesDuration + secondsDuration,
				end = parseInt( s.end );
			...
			if ( isNaN( totalDuration ) || totalDuration === 0 ) {
				errors.push( this.validation_error( 'days_duration', ThriveUlt.t.Invalid_value ) );
				errors.push( this.validation_error( 'hours_duration', '' ) );
				errors.push( this.validation_error( 'minutes_duration', '' ) );
				errors.push( this.validation_error( 'seconds_duration', '' ) );
			}

			if ( ! $.trim( s.days_duration ).match( /^\d+$/ ) || isNaN( daysDuration ) || daysDuration < 0 ) {
				errors.push( this.validation_error( 'days_duration', ThriveUlt.t.Invalid_duration ) );
			}
			if ( ! $.trim( s.hours_duration ).match( /^\d+$/ ) || isNaN( hoursDuration ) || hoursDuration > 23 || hoursDuration < 0 ) {
				errors.push( this.validation_error( 'hours_duration', ThriveUlt.t.Invalid_value_hours ) );
			}
			if ( ! $.trim( s.minutes_duration ).match( /^\d+$/ ) || isNaN( minutesDuration ) || minutesDuration > 59 || minutesDuration < 0 ) {
				errors.push( this.validation_error( 'minutes_duration', ThriveUlt.t.Invalid_value_minutes_seconds ) );
			}
			if ( ! $.trim( s.seconds_duration ).match( /^\d+$/ ) || isNaN( secondsDuration ) || secondsDuration > 59 || secondsDuration < 0 ) {
				errors.push( this.validation_error( 'seconds_duration', ThriveUlt.t.Invalid_value_minutes_seconds ) );
			}
		...
		getSummary: function () {
			...
			if ( type === ThriveUlt.util.campaignType.rolling ) {
				...
				var trigger = settings.trigger.type,
					on = '',
					days = '';
				if ( type === ThriveUlt.util.campaignType.evergreen ) {
					days = '';
					end = '';
					var d = ( typeof settings.days_duration === 'undefined' ? parseInt( settings.duration ) : parseInt( settings.days_duration ) ) || 0,
						h = parseInt( settings.hours_duration ) || 0,
						m = parseInt( settings.minutes_duration ) || 0,
						s = parseInt( settings.seconds_duration ) || 0;

					end = ( d > 0 ? d + ( d === 1 ? ' day ' : ' days ' ) : '' ) +
				...
				if ( trigger && trigger === ThriveUlt.util.triggerType.webhook ) {
					return this.set( 'summary',
						ThriveUlt.util.printf( ThriveUlt.integratedApis[ settings.trigger.api ].label === 'FluentCRM' ? ThriveUlt.t.SummaryStartsEvergreenTag : ThriveUlt.t.SummaryStartsEvergreenWebhook,
							[ ThriveUlt.integratedApis[ settings.trigger.api ].label ] ) );
				} else if ( trigger && trigger !== ThriveUlt.util.triggerType.webhook ) {
		...
		has_valid_lockdown_trigger: function () {
			...
			return [
				ThriveUlt.util.triggerType.conversion,
				ThriveUlt.util.triggerType.promotion,
				ThriveUlt.util.triggerType.webhook
			].includes( _settings.trigger.type );
		},
	 ...
	 * ThriveBox Model
	 */
	ThriveUlt.models.ThriveBoxModel = ThriveUlt.models.Base.extend( {} );

	/**
	 * Thrive Boxes collection
	 * @type {void|*}
	 */
	ThriveUlt.collections.ThriveBoxesCollection = ThriveUlt.collections.Base.extend( {
		model: ThriveUlt.models.ThriveBoxModel
	} );
	...
	ThriveUlt.models.Event = ThriveUlt.models.Base.extend( {
		...
		validateTrigger: function ( start_event ) {
			...
			var duration = parseInt( start_event.get( 'days' ) ) * 24 + parseInt( start_event.get( 'hours' ) ),
				...
				campaignSettings = ThriveUlt.globals.campaign.get( 'settings' );

			var d = ( typeof campaignSettings.days_duration === 'undefined' ? parseInt( campaignSettings.duration ) : parseInt( campaignSettings.days_duration ) ) || 0,
				h = parseInt( campaignSettings.hours_duration ) || 0,
				m = parseInt( campaignSettings.minutes_duration ) || 0,
				s = parseInt( campaignSettings.seconds_duration ) || 0,
				campaignDuration = d * 24 + h + m / 60 + s / 60 / 60;
			 ...
			 * Since the campaign duration for evergreen is more complex, an extra condition has been added
			 */
			if ( trigger >= duration || ( trigger >= campaignDuration && ThriveUlt.globals.campaign.get( 'type' ) === 'evergreen' ) ) {
		...
		getLabel: function () {
			...
			if ( this.get( 'hours' ) == 0 && this.get( 'days' ) == 0 ) {
				return '';
			}
			...
			renderSettings: function () {
				...
				if ( type == ThriveUlt.util.campaignType.evergreen ) {
					if ( ThriveUlt.data.lead_groups === false ) {
						...
						thriveBoxes = new ThriveUlt.collections.ThriveBoxesCollection( ThriveUlt.data.thrive_boxes );
					}
				}
				if ( ! this[ type ] ) {
					this[ type ] = new ThriveUlt.views[ 'CampaignType' + ThriveUlt.util.upperFirst( type ) + 'State' ]( {
						...
						thriveBoxes: thriveBoxes,
						thriveQuizzes: ThriveUlt.data.tqb_optin,
					} );
				} else {
			...
			renderSettings: function () {
				...
				if ( type == ThriveUlt.util.campaignType.evergreen ) {
					if ( ThriveUlt.data.lead_groups === false ) {
						...
						this.thriveBoxes = new ThriveUlt.collections.ThriveBoxesCollection( ThriveUlt.data.thrive_boxes );
					}
				}
		...
		ThriveUlt.views.ModalConversionEvent = ThriveUlt.views.ModalSteps.extend( {
			...
			renderTriggers: function () {
				var trigger = this.model.get( 'trigger_options' ).trigger,
					...
					thriveBoxes = new ThriveUlt.collections.ThriveBoxesCollection();
				...
				if ( ThriveUlt.data.thrive_boxes ) {
					thriveBoxes.reset( ThriveUlt.data.thrive_boxes );
				}
...
( function ( $ ) {
	...
	$( function () {
		...
		ThriveUlt.views.Dashboard = ThriveUlt.views.Base.extend( {
			...
			events: {
				'click .tvu-add-new-campaign': 'addNew'
			},
			addNew: function () {
		...
		ThriveUlt.views.LockdownServiceURL = ThriveUlt.views.Base.extend( {
			...
			className: 'tvd-copy-row control-grid tvd-no-mb tvu-url-container',
			initialize: function () {
		...
		ThriveUlt.views.CampaignTypeEvergreenState = ThriveUlt.views.Base.extend( {
			...
			template: TVE_Dash.tpl( 'campaign/type/evergreen-state' ),
			events: {
				'change .tvu-evergreen-settings': 'changeEvergreenState',
				'change .tvu-evergreen-duration': function ( event ) {
					var field = event.currentTarget.dataset.field,
						value = parseInt( event.currentTarget.value );

					this.model.get( 'settings' )[ field ] = isNaN( value ) ? 0 : value;
				},
			...
			initialize: function ( options ) {
				...
				this.thriveBoxes = options.thriveBoxes;
			},
			/**
			 * Set default settings in case something was not set
			 */
			prepareSettings: function () {
				var settings = this.model.get( 'settings' );

				if ( ( settings.trigger.type == ThriveUlt.util.trigger_type.conversion && ! this.shortcodes && ! this.collection ) || ! settings.trigger.type ) {
					settings.trigger.type = this.model.get( 'lockdown' ) ? 'promotion' : 'first';
				}

				if ( typeof settings.days_duration === 'undefined' ) {
					/* backwards compatibility for old campaigns */
					settings.days_duration = parseInt( settings.duration );
					settings.hours_duration = 0;
					settings.minutes_duration = 0;
					settings.seconds_duration = 0;
				}
			...
			render: function () {
				...
				this.$( '#tvu-evergreen-gmt-offset' ).on( 'change', function ( event ) {
					/* update settings on select change */
					self.get( 'settings' ).gmt_offset = event.currentTarget.value;
				} ).val( self.get( 'settings' ).gmt_offset || 'UTC+0' ).trigger( 'change' );
			...
			changeRealEnding: function ( e ) {
				var repeat = $( e.currentTarget ).val();

				this.$el.find( '.tvu-duration-label-realistic' ).toggleClass( 'hidden' );
				this.$el.find( '.tvu-duration-label-absolute' ).toggleClass( 'hidden' );
		...
		ThriveUlt.views.EvergreenConversionSettings = ThriveUlt.views.Base.extend( {
			...
			render: function () {
				this.$el.html( this.template( {
					...
					thriveBoxes: this.thriveBoxes,
					thriveQuizzes: ThriveUlt.data.tqb_optin,
					leadGenPages: ThriveUlt.data.lead_gen_pages,
					lightBoxes: ThriveUlt.data.tve_lightboxes
				} ) );
		 ...
		 * @type {void|*}
		 */
		ThriveUlt.views.EvergreenWebhookSettings = ThriveUlt.views.Base.extend( {
			className: 'tvu-trigger-option tvu-trigger-option-webhook',
			template: TVE_Dash.tpl( 'campaign/type/evergreen-webhook' ),
			integratedApis: ThriveUlt.integratedApis,
			events: {
				'change .tvu-webhook-api': 'changeAPi',
			},
			initialize: function ( options ) {
				this.shortcodes = options.shortcodes;
			},
			changeAPi: function ( event ) {
				var settings = this.model.get( 'settings' );
				settings.trigger.api = event.target.value;
				this.model.set( 'settings', settings );
				this.render();
			},
			render: function () {
				...
				if ( ! settings.trigger.api ) {
					settings.trigger.api = Object.keys( this.integratedApis )[ 0 ];
					this.model.set( 'settings', settings );
				}

				for ( var key in this.integratedApis ) {
					this.integratedApis[ key ].selected = settings.trigger.api === key;
				}
				var api = this.model.get( 'settings' ).trigger.api;
				this.$el.html( this.template() );

				if ( this.integratedApis[ api ] && this.integratedApis[ api ].custom_integration ) {

					this.$( '.tvu-api-integration' ).html( TVE_Dash.tpl( 'campaign/type/api-partials/' + api )( {
						item: api,
						kb_article: this.getKBAricle(),
					} ) );
					this[ api ]();
				} else {

					this.$( '.tvu-api-integration' ).html( TVE_Dash.tpl( 'campaign/type/api-partials/general' )( {
						item: api,
						integratedApis: this.integratedApis,
						url: this.generateURL(),
						kb_article: this.getKBAricle(),
					} ) );
				}

				var $select = this.$( '#tvu-webhook-api' );

				_.each( this.integratedApis, function ( option ) {
					var $optionLabelTag = option.label.endsWith( 'webhook' ) ? '' : ( option.label.endsWith( 'FluentCRM' ) ? ' tag' : ' webhook' );
					var $option = $( '<option value="' + option.key + '" data-image="' + option.image + '">' + option.label + $optionLabelTag + '</option>' );
					if ( option.selected ) {
				...
				$select.select2( {
					templateResult: ThriveUlt.util.select2ImageItemRenderer,
					templateSelection: ThriveUlt.util.select2ImageItemRenderer
				} );
			...
			fluentcrm: function () {
				this.$( '.tvu-fluentcrm-trigger-dropdown' ).select2();
				var self = this,
					settings = self.model.get( 'settings' ),
					existingTags = ThriveUlt.integratedApis.fluentcrm.data;

				existingTags.filter( function ( obj ) {
					obj.selected = settings.trigger.ids.includes( parseInt( obj.id ) );
				} )

				var $select = this.$( '#tvu-fluentcrm-trigger-tags' ).select2( {
					tags: true,
					data: existingTags,
					templateResult: function ( d ) {
						return $( '<div class="tvu-leads-dropdown-option"><span >' + d.text + '</span></div>' );
					}
				} ).on( 'change', function ( e ) {
					settings.trigger.ids = $select.select2( 'data' ).map( function ( el ) {
						return parseInt( el.id )
					} );
					self.model.set( 'settings', settings );
				} );
			},
			getKBAricle: function () {
				return this.integratedApis[ this.model.get( 'settings' ).trigger.api ].kb_article;
			},
			generateURL: function () {
				var code, settings = this.model.get( 'settings' );
				if ( settings.trigger.code ) {
					code = settings.trigger.code;
				} else {
					code = ( new Date().getTime() ).toString( 36 );
					settings.trigger.code = code;
					this.model.set( 'settings', settings );
				}

				return ThriveUlt.tvd_webhook_rest_url + settings.trigger.api + '/' + this.model.get( 'ID' ) + '/' + code;
			},
			get_trigger_description: function () {
				return '<em>' + ThriveUlt.t.trigger_webhook + '</em>';
			}
		} );
		...
		ThriveUlt.views.EvergreenCampaignCheckbox = ThriveUlt.views.Base.extend( {
			...
			render: function () {
				...
				//Shorten text to fit in desired width
				var labelText = this.$el.find( 'label' );
				labelText.text( ThriveUlt.util.trimStringAfterWidth( labelText.text(), 175, 16, 'Roboto', 400 ) );
		...
		ThriveUlt.views.DesignNormalState = ThriveUlt.views.Base.extend( {
			...
			className: 'tvu-design-item mr-30',
			events: {},
			render: function () {
				this.$el.html( this.template( {item: this.model} ) );
				return this;
		...
		ThriveUlt.views.TriggerTypeConversion = ThriveUlt.views.Base.extend( {
			...
			render: function () {
				...
				this.thriveBoxes.each( this.renderLeads, this );
			...
			renderLeads: function ( item ) {
				...
				} else if ( item.get( 'post_type' ) === ThriveUlt.util.leadtype.thriveBox ) {
					this.$el.find( '.tvu-conversion-thrivebox-checkboxes' ).append( v.render().$el );
				} else {
	...
	ThriveUlt.PostSearch = function ( $input, options ) {
		...
		$input.autocomplete( options ).data( "ui-autocomplete" )._renderItem = function ( ul, item ) {
			...
			return $( "<li class='" + _class + "'>" ).append( "<span class='tvu-ps-result-title tvd-truncate' style='max-width: 85%'>" + item.label + "</span><span class='tvu-ps-result-type'>" + item.type + "</span>" ).appendTo( ul );
		};
		...
		//Remove the errors from the input element
		$input.on( 'input', function ( event ) {
			var input = $( event.currentTarget );
			input.removeClass( 'tvd-invalid' );
			input.siblings().removeAttr( 'data-error' );
		} );
	...
	ThriveUlt.LeadGroupSearch2 = function ( $el, options, mainModel ) {
		var selectFormsTriggers = [];
		var $select = $el.find( '#tvu-trigger-form-ids' ),
			categories = {
				'tve_lead_group': 'Lead Group',
				'tve_lead_shortcode': 'Shortcode',
				'tve_lead_2s_lightbox': 'Thrive Box',
			};
		...
		options.forEach( function ( el ) {
			el.models && el.models.forEach( function ( part, index ) {
				selectFormsTriggers.push( {
					text: shortTitle( part.attributes.post_title ),
					subText: categories[ part.attributes.post_type ],
					id: part.attributes.ID,
					selected: mainModel.get( 'settings' ).trigger.ids.indexOf( part.attributes.ID ) > - 1
				} );
			} );
		} );

		ThriveUlt.data.tqb_optin.forEach( function ( el ) {
			if ( el.optin && el.quiz_title ) {
				selectFormsTriggers.push( {
					text: shortTitle( el.quiz_title ),
					subText: 'Quiz',
					id: el.optin,
					selected: mainModel.get( 'settings' ).trigger.ids.indexOf( el.optin ) > - 1
				} );
			}
		} );

		if ( ThriveUlt.data.lead_gen_pages ) {
			_.each( ThriveUlt.data.lead_gen_pages, function ( page, id ) {
				selectFormsTriggers.push( {
					text: shortTitle( page ),
					subText: 'Page',
					id: id,
					selected: mainModel.get( 'settings' ).trigger.ids.indexOf( parseInt( id ) ) > - 1
				} );
			} )
		}
		if ( ThriveUlt.data.tve_lightboxes ) {
			_.each( ThriveUlt.data.tve_lightboxes, function ( page, id ) {
				selectFormsTriggers.push( {
					text: shortTitle( page ),
					subText: 'Thrive Lightbox',
					id: id,
					selected: mainModel.get( 'settings' ).trigger.ids.indexOf( parseInt( id ) ) > - 1
				} );
			} )
		...
		if ( $select.length ) {
			$select.select2( {
				tags: true,
				data: selectFormsTriggers,
				templateResult: function ( d ) {
					return $( '<div class="tvu-leads-dropdown-option"><span >' + d.text + '</span><span class="tvu-leads-dropdown-category">' + d.subText + '</span></div>' );
				}
			} ).on( 'change', function ( e ) {
				mainModel.get( 'settings' ).trigger.ids = $select.select2( 'data' ).map( function ( el ) {
					return parseInt( el.id )
				} );
			} );
		}
