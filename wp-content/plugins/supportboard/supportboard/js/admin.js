        ...
        dialogflow: {
            ...
            submitIntent: function (button) {
                ...
                if ((!response && !intent_name) || expressions.length == 0) {
                    ...
                    SBF.ajax({
                        function: is_dialogflow_disabled ? 'embeddings-database' : (!intent_name ? 'dialogflow-create-intent' : 'dialogflow-update-intent'),
        ...
        whatsapp: {
            ...
            send: function (to, message = '', attachments = [], phone_id = false, onSuccess = false) {
                SBF.ajax({
                    ...
                    if (response.error) infoPanel(response.error.message, 'info', false, 'error-wa');
                    if (onSuccess) onSuccess(response);
    ...
    */
    ...
    var SBConversations = {
        ...
        messageMenu: function (agent, message = false) {
            let code = (message && SBApps.is('dialogflow') ? `<li data-value="bot">${sb_('Train chatbot')}</li>` : '') + ((agent && !SB_ADMIN_SETTINGS.supervisor && SB_ADMIN_SETTINGS.allow_agent_delete_message) || (SB_ADMIN_SETTINGS.supervisor && SB_ADMIN_SETTINGS.allow_supervisor_delete_message) ? `<li data-value="delete">${sb_('Delete')}</li>` : '');
    ...
    $(document).ready(function () {
        ...
        */
        ...
        notes_panel.on('click', '> i', function (e) {
            SBAdmin.genericPanel('notes', 'Add new note', `<div class="sb-input-setting sb-type-textarea"><textarea placeholder="${sb_('Write here your note...')}"></textarea></div>`, [['Add note', 'plus']]);
            e.preventDefault();
            return false;
        });
            ...
            if (message.length == 0) {
                ...
                SBConversations.notes.add(SBChat.conversation.id, SB_ACTIVE_AGENT.id, SB_ACTIVE_AGENT['full_name'], message, (response) => {
                    if (Number.isInteger(response)) {
                        ...
                        textarea.val('');
                        infoBottom('New note successfully added.');
                    } else {
        ...
        tags_panel.on('click', '> i', function (e) {
            let code = '<div class="sb-tags-cnt">';
            let tags = SBChat.conversation.details.tags;
            let add = '<i id="sb-add-tag" class="sb-icon-plus sb-btn-icon"></i></div>';
            for (var i = 0; i < tags.length; i++) {
            ...
            if (!SBConversations.tags.list) {
                ...
                SBF.ajax({
                    ...
                    SBAdmin.genericPanel('tags', 'Manage tags', code + SBConversations.tags.getAll(tags) + add, ['Save tags']);
                });
            } else {
                SBAdmin.genericPanel('tags', 'Manage tags', code + SBConversations.tags.getAll(tags) + add, ['Save tags']);
            }
        ...
        */
        ...
        $(settings_area).on('click', '#sb-import-settings a', function (e) {
            ...
            upload_on_success = (response) => {
                ...
                SBF.ajax({
                    ...
                    if (response) infoBottom('Settings successfully imported. Reload the admin area to apply the new settings.');
                    else infoPanel(response, 'info');
                    $(this).sbLoading(false);
                });
        ...
        $(settings_area).on('click', '#whatsapp-twilio-btn .sb-btn, #sms-btn .sb-btn, #wechat-btn .sb-btn, #twitter-callback .sb-btn, #gbm-webhook .sb-btn, #viber-webhook .sb-btn, #line-webhook .sb-btn', function (e) {
            ...
            infoPanel(`<pre>${SB_URL + (id == 'sms-btn' ? '/include/api.php' : ('/apps/' + id.substring(0, id.indexOf('-')) + '/post.php')) + cloudURL().replace('&', '?')}</pre>`, 'info');
            return false;
            e.preventDefault();
        });
