(function ($) {
	...
	var RX_Admin = {
		...
		bindEvents: function () {
			...
			$('body').delegate('.rx-media-field-wrapper .rx-media-upload-button', 'click', function (e) {
				e.preventDefault();
				RX_Admin.initMediaField(this);
			});
			$('body').delegate('.rx-media-field-wrapper .rx-media-remove-button', 'click', function (e) {
				e.preventDefault();
				RX_Admin.removeMedia(this);
			});
		...
		tabChanger: function (button) {
			...
			if (RX_Admin.onCheckCriteria() === true) {
				if ('.rx-metatab-menu li[data-tabid="' + tabID + '"]' != undefined) {
		...
		selectImage: function (image) {
			...
			var imgParent = $(image),
				...
				wrapper = $(imgParent.parent().parents('.rx-theme-field-wrapper')),
				inputID = wrapper.data('name');
			imgParent.addClass('rx-theme-selected').parent().siblings().find('.rx-single-theme-wrapper').removeClass('rx-theme-selected');
			$('#' + inputID).val(value);
		...
		fieldAlert: function (element) {
			...
			premium_anchor.setAttribute('href', 'https://jouleslabs.com/go/reviewx');
	...
	function importAjax() {
		const importSubmit = $("#rv-import-submit");
		const importBtn = $(".reviewx-import-submit-btn");
		const stepNext = $(".step-next");
		const firstStep = $(".rx-import-step-first");
		const thirdStep = $(".rx-import-step-third");
		const warningMsg = $(".rvx-import-step__warning-msg");

		importSubmit.on("submit", function (e) {
			e.preventDefault();

			const mainFile = $("#rvx-import-file");
			...
			const csvFile = formData.get('reviewx_import_review_file');
			formData.append("security", ajax_admin.ajax_nonce);
			formData.append("csv_file", csvFile);
			formData.append('action', 'rx_import_review');
			const formState = importSubmit.attr('rv-state');
			formData.append('form_state', formState);

			// showing loader while importing review
			importLoader(formState)
			...
			$.ajax({
				...
				data: formData,
				success: function (res) {
					if (res && firstStep.hasClass('active')) {
						$('.rx-import-step-second').html(res);
						stepNext.click();
						create_data_group_for_import();
					}
					if (res && thirdStep.hasClass('active')) {
						stepNext.click();
						$('.rx-import-step-forth').html(res);
					}
					...
					if (formState == 3 && res) {
						...
						//send response for split files
						for (let i = 1; i <= response.total_file_count; i++) {
							split_file(response, formData, i);
						}

						let responseSuccessMsg = '';
						let responseErrorMsg = '';
						message.forEach(function (item) {
							if (item.success) {
								responseSuccessMsg = `<strong>${item.success}</strong>`;
							} else {
								responseErrorMsg = `<br>${item.error}`;
							}
						});
						console.log(responseSuccessMsg, responseErrorMsg);

						Swal.fire({
							title: 'Complete!',
							text: 'Your file has been imported.',
							//html: `<p>${responseSuccessMsg} ${responseErrorMsg}</p>`,
							type: 'success',
							timer: 3000,
						}).then(function () {
	...
	function split_file(resParse, formData, i) {
		if (resParse.total_file_count) {

			var currentFile = i;

			if (currentFile <= resParse.total_file_count) {

				formData.append('rx_file_name', currentFile);
	...
	function importLoader(state) {
		const from = $("#rv-import-submit");
		const progressButton = $(".rv-step-button");
		const fromProgress = $(".form-progress");
		const deviderBottom = $(".rvx-import-page__devider-bottom-container");
		const buttonNextBack = $(".rvx-import-page__button-container");
		const loader = $(".rx-loader-wrap");
		const fromBoder = $(".rvx-import-page__main-container");

		if (state == 3) {
			progressButton.addClass("hide");
			from.addClass("hide");
	...
	function create_data_group_for_import() {
		const foreignProductId = $("#rvx-foreign-product-id");
		const thirdStep = $(".rx-import-step-third");
		const stepNext = $(".step-next");

		foreignProductId.on("change", function (e) {
			$.ajax({
				url: ajax_admin.ajax_admin_url,
				type: 'POST',
				data: {
					"action": 'create_array_group_by_product',
					"security": ajax_admin.ajax_nonce,
					"productID": foreignProductId.val()
				},
				success: function (res) {
					if (res) {
						thirdStep.html(res);
	...
	const fileChosen = document.getElementById('rvx-file-chosen');
	$("#rvx-import-file").on("change", function () {
		var file = this.files[0],
			fileName = file.name;
		fileChosen.textContent = fileName
	});

	function formStepHandeler() {
		const importSubmit = $("#rv-import-submit");
		const stepNext = $(".step-next");
		const stepBack = $(".step-back");
		const firstStep = $(".rx-import-step-first");
		const secondStep = $(".rx-import-step-second");
		const thirdStep = $(".rx-import-step-third");
		const forthStep = $(".rx-import-step-forth");
		const progressBar = $(".rvx-form-progress-bar");
		const progressIndicator = $(".rvx-form-progress-indicator");
		const submitBtn = $(".reviewx-import-submit-btn");
		const submitBtnAsNext = $(".rvx-import-page__step-next .reviewx-import-submit-btn")
		submitBtnAsNext.css('display', 'none')
		let mainState = 0;
		let state = 0;
		let backState = 0;
		...
		stepNext.on("click", function (e) {
			state++;
			let progressVal = progressBar.val()
			progressBar.val(progressVal + 33);

			if (state > 1) {
				stepBack.removeAttr("disabled")
				stepBack.addClass('active')
			}

			if (state == 1) {
				$(progressIndicator[1]).addClass("active")
				submitBtn.removeClass("active")

				firstStep.removeClass("active")
				secondStep.addClass("active")
			} else if (state == 2) {
				$(progressIndicator[2]).addClass("active")
				submitBtn.css('display', 'none')
				stepNext.css('display', 'none')
				submitBtnAsNext.css('display', 'block')

				submitBtnAsNext.addClass("active rx-third-stage")
				submitBtnAsNext.val("Next")


				secondStep.removeClass("active")
				thirdStep.addClass("active")
			} else if (state == 3) {
				$(progressIndicator[3]).addClass("active")
				submitBtn.css('display', 'none')
				stepNext.css('display', 'none')
				submitBtnAsNext.css('display', 'block')
				submitBtn.addClass("active rx-forth-stage")
				submitBtn.val("Import")
				
				stepNext.attr("disabled", "disabled")
				thirdStep.removeClass("active")
				forthStep.addClass("active")
			}
			...
			importSubmit.attr("rv-state", state);
		});
		...
		stepBack.on("click", function (e) {
			...
			if (mainState === state) {
				...
				// stepNext.attr("disabled", "disabled")
				// state = 2
				// importSubmit.removeAttr("rv-state", 2)
			}
			importSubmit.attr("rv-state", state);
		});
	}

	// imported review delete ajax
	function historyDeleteAjax() {
		const revertButton = $(".rvx-import-revert-btn");
	...
	$(document).ready(function ($) {
		...
		$('.rx_save_setting_tab', document).on('click', function (e) {
			let that = this;
			//Grab current tab
			let $tab = $(this).data('tab');
			...
			if (RX_Admin.onCheckCriteria() === true) {
				...
				if ($("#rx_builder_current_page").val() == 'reviewx-review-email') {
					rx_tab_field_val.push({ name: 'rx_option_unsubscribe_url', value: $("#rx_option_unsubscribe_url").val() });
				}
				...
				if (rxs[0].value == "email_tab") {
					...
					$.ajax({
						...
						data: {
							...
							filter: filterIntialization('', true)
						},
						success: function (data) {
							$('#TB_overlay').click();
		...
		//Check criteria field is not empty
	...
	$(document).ready(function ($) {
		...
		let addButton = $('.rx-click-overlay'); //Add button selector
			...
			if (total < maxField) { //Check maximum number of input fields
				...
				if ($('#rx_builder_current_page').val() == 'post-type-reviewx') {
					fieldHTML = '<tr valign="top"><td class="middle-align">' +
						'<input type="text" name="rx_meta_review_criteria[]" value="" class="timezone_string rx-meta-field playfield" placeholder="Criteria Name"/><input type="hidden" name="rx_meta_review_criteria_name[]" value="' + criteria_name + '"/>' +
					...
					fieldHTML = '<tr valign="top"><td class="middle-align">' +
						'<input type="text" name="rx_option_review_criteria[]" value="" class="timezone_string rx-meta-field playfield" placeholder="Criteria Name"/><input type="hidden" name="rx_option_review_criteria_name[]" value="' + criteria_name + '"/>' +
						'<a href="javascript:void(0);" class="up_button" title="Add"><svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 100 100" style="enable-background:new 0 0 100 100;" xml:space="preserve"><style type="text/css">.st0{fill:#9BA1B0;}</style><path class="st0" d="M99.1,47.9L82.7,32.5c-0.8-0.8-2.1-1-3.1-0.5c-1.1,0.5-1.7,1.5-1.7,2.6v6.7H58.7V22.1h6.7 c1.2,0,2.2-0.7,2.6-1.7c0.5-1.1,0.2-2.3-0.5-3.1L52.1,0.9C51.6,0.3,50.8,0,50,0s-1.6,0.3-2.1,0.9L32.5,17.3c-0.8,0.8-1,2.1-0.5,3.1 c0.5,1.1,1.5,1.7,2.6,1.7h6.7v19.2H22.1v-6.7c0-1.2-0.7-2.2-1.7-2.6c-1.1-0.5-2.3-0.2-3.1,0.5L0.9,47.9C0.3,48.4,0,49.2,0,50 s0.3,1.6,0.9,2.1l16.3,15.4c0.8,0.8,2.1,1,3.1,0.5c1.1-0.5,1.7-1.5,1.7-2.6v-6.7h19.2v19.2h-6.7c-1.2,0-2.2,0.7-2.6,1.7 c-0.5,1.1-0.2,2.3,0.5,3.1l15.4,16.3c0.5,0.6,1.3,0.9,2.1,0.9s1.6-0.3,2.1-0.9l15.4-16.3c0.8-0.8,1-2.1,0.5-3.1 c-0.5-1.1-1.5-1.7-2.6-1.7h-6.7V58.7h19.2v6.7c0,1.2,0.7,2.2,1.7,2.6c1.1,0.5,2.3,0.2,3.1-0.5l16.3-15.4c0.6-0.5,0.9-1.3,0.9-2.1 S99.7,48.4,99.1,47.9z"/><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g></svg></a>' +
						'<a href="javascript:void(0);" class="remove_button" title="Remove"><svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 100 100" style="enable-background:new 0 0 100 100;" xml:space="preserve"><g><path class="st0" d="M19,35.3c1.4,20.8,2.7,37.6,4.1,58.4c0.2,3.6,3.2,6.2,6.9,6.2c13.2,0,26.5,0,39.7,0c3.9,0,6.7-2.7,7-6.7 C77.8,76,79,58.8,80.1,41.5c0.3-3.8,0.4-3.6,0.7-7.6c-20.7,0-41.2,0-61.7,0C19,34.4,19,34.9,19,35.3z M61.7,56 c0.2-2.4,0.4-4.7,0.6-7.1c0.2-2.4,1.5-3.7,3.4-3.5c1.9,0.2,3,1.6,2.8,4c-0.6,7.4-1.3,14.8-1.9,22.2c-0.4,4.1-0.7,8.3-1.1,12.4 c-0.2,2.2-1.5,3.5-3.3,3.4c-1.8-0.1-3-1.6-2.9-3.2C60.1,74.5,60.9,65.2,61.7,56z M46.7,49.3c0-2.6,1.2-4,3.2-3.9 c1.9,0.1,3,1.4,3,3.9c0,5.7,0,11.5,0,17.2c0,0,0,0,0,0c0,5.7,0,11.3,0,17c0,2.5-1.2,4.1-3.1,4.1c-1.9,0-3.1-1.5-3.1-4 C46.7,72,46.7,60.7,46.7,49.3z M32.8,45.7c2.1-1.1,4.3,0.3,4.6,2.9c0.6,6.7,1.2,13.5,1.8,20.2c0.4,5,0.9,9.9,1.3,15.6 c0,1.4-1.1,2.9-2.9,3.1c-1.8,0.2-3.2-1.2-3.4-3.3c-1-11.5-2-23.1-3.1-34.6C31,47.9,31.3,46.5,32.8,45.7z"/><path class="st0" d="M88.7,16.3c-0.3-2.7-2.6-4.8-5.4-5.2c-1.1-0.2-2.2-0.2-3.3-0.2c-4.2,0-8.4,0-12.9,0c0-1.4,0-2.6,0-3.8 C66.9,2.7,64.2,0,59.7,0c-6.6,0-13.2,0-19.8,0c-4.2,0-7,2.7-7.2,6.9c-0.1,1.2,0,2.5,0,3.9c-1.1,0-1.9,0.1-2.7,0.1c-4,0-8-0.1-12,0 c-3.3,0.1-6.1,1.7-6.7,4.5c-0.6,2.5-0.4,5.2-0.6,7.9c26.3,0,52.1,0,78.1,0C88.7,20.9,88.9,18.6,88.7,16.3z"/></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g></svg></a>' +
						'<div class="show_error_message"></div>' +
						'</div></td></tr>';
		...
		/**
		 * When make any change and click previous/next button
		 */
		$(".quick-builder-submit-btn").on('click', function () {
			RX_Admin.onLoadSettings('option');
		});
	});
	...
	$(document).ready(function () {
		 ...
		 *========================================*/
		if (!$('body').hasClass('reviewx_page_rx-add-review')) {
			rx_image_popup();
		}
	...
	$('.quick-builder-finalize-btn').on('click', function () {
		...
		Swal.fire({
			title: ajax_admin.rx_finalize_title,
			text: '',
			html: ajax_admin.rx_finalize_msg,
			type: 'success',
			timer: 2000,
		}).then(function () {
	...
	$('#rx-admin-upload-photo').on('click', function (event) {
		...
		file_frame.on('select', function () {
			...
			let wrapper = $('.rx_edit_review_photos'); //Input image wrapper
			$('.rx_edit_review_photos').removeClass('rx-hide');
			$.each(attachment, function (index, value) {
				$(wrapper).prepend('<div class="rx_photo"><div class="popup-link"><a href="' + value.url + '"><img src="' + value.url + '" class="img-fluid" alt=""></a></div><div class="rx_remove_images"><a href="javascript:void(0);" class="rx_admin_remove_image"><svg style="width: 15px" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="times-circle" class="svg-inline--fa fa-times-circle fa-w-16" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path style="fill: #d21212" fill="currentColor" d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8zm121.6 313.1c4.7 4.7 4.7 12.3 0 17L338 377.6c-4.7 4.7-12.3 4.7-17 0L256 312l-65.1 65.6c-4.7 4.7-12.3 4.7-17 0L134.4 338c-4.7-4.7-4.7-12.3 0-17l65.6-65-65.6-65.1c-4.7-4.7-4.7-12.3 0-17l39.6-39.6c4.7-4.7 12.3-4.7 17 0l65 65.7 65.1-65.6c4.7-4.7 12.3-4.7 17 0l39.6 39.6c4.7 4.7 4.7 12.3 0 17L312 256l65.6 65.1z"></path></svg></a></div><input type="hidden" name="rx-image[]" class="rx-image" value="' + value.id + '"></div>'); // display image

				let review_id = getUrlParameter('c');
				jQuery.ajax({
					url: ajax_admin.ajax_admin_url,
					type: 'post',
					data: {
						action: 'rx_admin_add_image',
						img_id: value.id,
						review_id: review_id,
						security: $("#rx-edit-review-nonce").val()
					},
					success: function (data) { },
					error: function (err) {
						console.log(err);
	...
	$(document).on('click', '.rx_admin_remove_image', function (e) { //Once remove button is clicked
		e.preventDefault();
		let $that = $(this);
		let img_id = $(this).parent().siblings(".rx-image").val();
		let review_id = getUrlParameter('c');
		...
		jQuery.ajax({
			...
			type: 'post',
			data: {
				action: 'rx_admin_remove_image',
				img_id: img_id,
				review_id: review_id,
				security: $("#rx-edit-review-nonce").val()
			},
			success: function (data) {
				$($that).parent().parent().remove(); //Remove image
			},
			error: function (err) {
				console.log(err);
			}
	...
	$(document).on('click', '.rx_admin_remove_video', function (e) { //Once remove button is clicked
		e.preventDefault();
		let $that = $(this);
		let review_id = getUrlParameter('c');
		jQuery.ajax({
	...
	$(document).on('change', '.rx-edit-video-source-control', function (e) { //Change admin edit review source controller
		let video_source = $(this).val();
		if (video_source == "self") {
			$('.rx-edit-review-video-external').hide();
			if ($('#rx-edit-review-video-self-player').attr('src') != '') {
				$('.rx-edit-review-video-self').show();
			}
			$('.rx-edit-review-video-uploader').show();
		} else {
			$('.rx-edit-review-video-self').hide();
			$('.rx-edit-review-video-uploader').hide();
			$('.rx-edit-review-video-external').show();
		}
	});

	$(document).on('click', '.rx-edit-review-video-uploader', function (e) { //Change admin edit review source controller
		e.preventDefault();
		...
		media_uploader.on("add-source", function () {
			...
			document.getElementById("rx_admin_edit_review_video_url_self").value = video_url;
			$('.rx-edit-review-video-self').show();
			$('#rx-edit-review-video-self-player').attr('src', video_url);
				...
				$.ajax({
					...
					success: function (data) {
						Swal.fire({
							title: 'Test Mail',
							text: '',
							html: 'Test mail sent successfully!',
							type: 'success',
							timer: 2000,
						})
					},
	...
	$('.rx_review_email_send_bulk_btn').on('click', function () {
	...
	if ($('#rx_option_auto_review_reminder').is(':checked')) {
		$("#rx-option-get_review_email").show();
	}

	$('#rx_option_auto_review_reminder').on('click', function () {
		if ($(this).is(':checked')) {
			$("#rx-option-get_review_email").fadeIn();
		} else {
			$("#rx-option-get_review_email").fadeOut();
	...
	function filterIntialization(prefix = '', freeze = false, clear = false) {
		...
		let reviewx_product = $('#manual_review_product');
		let reviewx_user = $('#manual_review_user');

		reviewx_product.select2({
			placeholder: ajax_admin.rx_select_product,
			multiple: false,
			allowClear: true,
		});

		reviewx_user.select2({
			placeholder: ajax_admin.rx_select_user,
			multiple: false,
			allowClear: true,
		});
		...
		let data = {
			...
			'start_date': startTimeElem.val(),
			'end_date': endTimeElem.val(),
			'filter_by': filterByElem.val(),
			'categories': filterByCategoriesElem.val(),
			'products': filterByProductsElem.val(),
			'special_condition': filterByConditionsElem.val()
		};
		...
		if (clear) {
			...
			filterProductsElem.prop('checked', false);

			filterByElem.val('');

			filterByCategoriesElem.val('').trigger('change');

			filterByProductsElem.val('').trigger('change');
	...
	function filterSchedule() {
		...
		return Object.assign(filterIntialization('schedule_', true), {
	...
	if ($('#rx_option_allow_video').is(':checked')) {
		$("#rx-option-video_source").show();
	}

	$('#rx_option_allow_video').on('click', function () {
		if ($(this).is(':checked')) {
			$("#rx-option-video_source").fadeIn();
		} else {
			$("#rx-option-video_source").fadeOut();
		}
	});

	if ($('#rx_meta_allow_video').is(':checked')) {
		$("#rx-option-video_source").show();
	}

	$('#rx_meta_allow_video').on('click', function () {
		if ($(this).is(':checked')) {
			$("#rx-option-video_source").fadeIn();
		} else {
			$("#rx-option-video_source").fadeOut();
	...
	$(document).on('change', "#manual_review_user", function (e) {
		let user_name = $(this).val();
		if (user_name == 'custom') {
			$('.manual-review-author').show();
		} else {
			$('.manual-review-author').hide();
		}
	});
	...
	$(document).ready(function () {
		...
		let attachment;
		// attach a click event (or whatever you want) to some element on your page
		$('#manual_review_photo').on('click', function (event) {
			event.preventDefault();
			if (file_frame) {
				file_frame.open();
				return;
	...
	$(document).on('change', '#manual_review_video_source', function (e) { //Change admin edit review source controller
		let video_source = $(this).val();
		if (video_source == "self") {
			$('#manual_review_self_video').show();
			$('#manual_review_note_video').hide();
			$('#manual_review_external_video_url').hide();
		} else {
			$('#manual_review_self_video').hide();
			$('#rx-show-video-preview').hide();
			$('#manual_review_note_video').show();
			$('#manual_review_external_video_url').show();
		}
	});

	$(document).on('click', '#manual_review_video', function (e) { //Change admin edit review source controller
		e.preventDefault();
		let media_uploader = wp.media({
			frame: "video",
			state: "add-video-source",
		...
		media_uploader.on("add-source", function () {
			let video_url = media_uploader.state().media.attachment.attributes.url;
			document.getElementById("manual_review_video_url").value = video_url;
			$('#rx-show-video-preview').show();
			$('#rx-show-video-preview').attr('src', video_url);
			media_uploader.close();
		});
		media_uploader.open();
	});

	$(document).on('keyup', '#manual_review_set_video_url', function () {
		let external_url = $(this).val();
		$("#manual_review_video_url").val(external_url);
	})

	$(document).on('click', '#save_manual_review', function (e) {

		let requireds = ['manual_review_product', 'manual_review_user', 'manual_review_text'];
		let formInput = $("#attachmentForm input");	 // grab all input field
		let data = formInput.serializeArray();
		let that = $(this);

		data.push({
			name: 'manual_review_product',
			value: $('#manual_review_product').val()
		});
		data.push({
			name: 'manual_review_user',
			value: $('#manual_review_user').val()
		});
		data.push({
			name: 'manual_review_text',
			value: tinyMCE.activeEditor.getContent()
		});
		data.push({
			name: 'manual_review_video_source',
			value: $('#manual_review_video_source').val()
		});

		if (check_required_fields(data, requireds) == false) {
			return;
		}

		that.text(ajax_admin.rx_setting_saving);
		that.attr("disabled", true);
		jQuery.ajax({
			url: ajax_admin.ajax_admin_url,
			type: 'post',
			data: {
				action: 'reviewx_manual_review',
				formInput: data,
				security: ajax_admin.ajax_nonce
			},
			success: function (data) {
				that.prop("disabled", false);
				that.text(ajax_admin.rx_save_setting);
				if (data.success == true) {
					Swal.fire({
						title: ajax_admin.review_success_title,
						text: '',
						html: ajax_admin.review_added_sucess,
						type: 'success',
						timer: 2000,
					}).then(function () {
						window.location.reload()
			...
			error: function (err) {
				...
				Swal.fire({
					title: ajax_admin.review_failed_title,
					text: '',
					html: ajax_admin.review_submit_failed,
					type: 'error',
					timer: 1000,
				}).then(function () {
	...
	function check_required_fields(data, requireds) {
		let status = true;
		data.forEach(function (item, index) {
			if( item.name != 'rx-image[]' ){
				$('#' + item.name + '').closest('.rx-form-item').find('.rx-input-error').text('');
			}
			requireds.forEach(function (require, index) {
				if (item.name == require && !item.value) {
					status = false;
					$('#' + item.name + '').closest('.rx-form-item').find('.rx-input-error').text('This field is required')
				}

				if (item.name == 'manual_review_user' && item.value == 'custom') {
				$('#' + item.name + '').parents().siblings().find('.rx-input-email-error').text('');
				$('#' + item.name + '').parents().siblings().find('.rx-input-name-error').text('');
					if ($('#manual_review_author').val() == '') {
						status = false;
						$('#' + item.name + '').parents().siblings().find('.rx-input-name-error').text('This field is required');
					}
					if ($('#manual_review_author_email').val() == '') {
						status = false;
						$('#' + item.name + '').parents().siblings().find('.rx-input-email-error').text('This field is required');
					}
				}
			});
		});
	...
	$(document).on('change', '#manual_review_product', function (e) {
		let that = $('#save_manual_review');
		that.attr("disabled", true);
		jQuery.ajax({
			url: ajax_admin.ajax_admin_url,
			type: 'post',
			data: {
				action: 'load_post_type_wise_criteria',
				post_id: $(this).val(),
				security: ajax_admin.ajax_nonce
			},
			success: function (data) {
				that.prop("disabled", false);
				$('#rx-manual-review-criteria').html(data);
			},
			error: function (err) {
				that.prop("disabled", false);
				console.log(err);
	...
	$('#manual_review_custom_date').on('click', function () {
		let that = $(this);
		if (that.prop("checked") == true) {
			$('.manual_review_date_area').fadeIn('medium');
		} else {
			$('.manual_review_date_area').fadeOut('medium');
		}
