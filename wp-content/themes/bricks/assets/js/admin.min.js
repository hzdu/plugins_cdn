...
/**
 * Hide regenerate CSS files admin notice after theme udpate when clicking dismiss button
 */
function bricksAdminGenerateCssFilesDismissNotice() {
	var dismissNotice = document.querySelector('#bricks-dismiss-regenerate-css-files .notice-dismiss')

	if (!dismissNotice) {
		return
	}

	dismissNotice.addEventListener('click', function (e) {
		jQuery.ajax({
			type: 'POST',
			url: bricksData.ajaxUrl,
			data: {
				action: 'bricks_dismiss_admin_notice_regenerate_css_files'
			},
			success: function (res) {
				console.info('bricks_dismiss_admin_notice_regenerate_css_files', res.data)
			}
		})
	})
}
...
document.addEventListener('DOMContentLoaded', function (e) {
	...
	setTimeout(function () {
		bricksAdminGenerateCssFilesDismissNotice()
	}, 400)
})
